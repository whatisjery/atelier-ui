...
Set `scrollDistance` and the component pins the canvas on screen while that much page scrolls past. This is what the demo above uses.

```tsx
<PixelScroll scrollDistance="300vh" direction="sweep" className="bg-black text-white" />
```

### Custom scroll section

If you need control over the layout, you can also build the section yourself and pass `scrollTargetRef`:
...
<Alert icon={<TriangleAlert strokeWidth={1.5} className="size-6" />} title="Section height" variant="neutral">
  When pinning, the scroll section must be taller than `100vh`, otherwise the grid snaps straight to filled.
</Alert>
...
| `scrollDistance` | `string` | — | When set, pins the canvas and drives the reveal over that much scrolling. Any CSS height. Ignored when `scrollTargetRef` is set. |
