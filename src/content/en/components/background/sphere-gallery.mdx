...
Sphere Gallery must be rendered inside a `WebglProvider`. Follow the [installation guide](/docs/getting-started/installation) to add it to your app root.
...
The gallery owns no DOM overlay. Use `onActiveChange` to get the focused index (already resolved, no modulo) and render your own content from your own data: a caption, a link, a title, anything.
...
`reveal` defaults to `true` so the sphere animates in on mount. Hold it `false` to delay the reveal, for example until an intro finishes.

```tsx title="Deferred reveal"
const [reveal, setReveal] = useState(false)

<>
  <Intro onDone={() => setReveal(true)} />
  <SphereGallery items={ITEMS} reveal={reveal} className="fixed inset-0" />
</>
```
