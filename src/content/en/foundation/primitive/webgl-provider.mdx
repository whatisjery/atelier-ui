...
The canvas runs on Motion's clock instead of its own `requestAnimationFrame`.
With [Smooth Scroll](/docs/foundation/primitive/smooth-scroll) on the page, every tick moves the scroll first, then the page, then renders the canvas, so WebGL elements always stay exactly on top of the elements they follow.

Add the provider once, at your application root. For the systems it belongs to, see the [introduction](/docs/getting-started/how-it-works).

---

## Install

<InstallGuide name="webgl-provider" />
