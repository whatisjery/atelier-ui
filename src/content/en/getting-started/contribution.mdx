...
You can add interactive controls to the preview so users can tweak component props live. Pass a `controls` object to `<DocComponentPreview />`.  

Always make sure the values match the default props of the base component:

```tsx title="Controls example"
<DocComponentPreview
...
export default function ComponentNameDemo(controls: Partial<ComponentNameProps>) {
    return 
}
```
...
Provide a basic usage example of the component:
