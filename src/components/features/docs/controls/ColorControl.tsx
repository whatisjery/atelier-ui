...
export default function ColorControl({ label, value, onChange }: ColorControlProps) {
    ...
    return (
        <div ref={ref} className="flex items-center relative p-3">
            <div className="w-full flex items-center justify-between">
                <span className="flex items-center gap-4">
                    <button
                        type="button"
                        onClick={() => (open ? close() : setOpen(true))}
                        className="size-10 cursor-pointer border hover:opacity-80 rounded flex items-center justify-center bg-mat-5"
                    >
