...
export default function PixelTrailDemo() {
    ...
    return (
        <div className="w-full h-full absolute inset-0 not-prose flex items-center justify-center">
            ...
            <MagneticDotGrid
                spacing={theme === "dark" ? 17 : 12}
                strength={theme === "dark" ? 15 : 20}
                dotRadius={0.8}
                snapSpeed={theme === "dark" ? 7 : 10}
                returnSpeed={theme === "dark" ? 4 : 6}
                floatAmplitude={2.2}
                interactionRadius={300}
                opacityRange={theme === "dark" ? [1, 0.1] : [1, 1]}
                baseColor={theme === "dark" ? "#3C4051" : "#979AAC"}
                centerColors={theme === "dark" ? ["#00FF73", "#00D9FF"] : ["#000000"]}
                cycleSpeed={0.5}
