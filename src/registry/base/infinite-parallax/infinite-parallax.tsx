...
/*
 * Renders children invisibly for initial size measurement.
 */
function TempMeasurementDiv({
    containerRef,
    childArray,
    className,
}: {
 ...
 */
function Parallax({ children, speed = 1, className, containerRef }: ParallaxColumnProps) {
    ...
    const y = useTransform(scrollYProgress, [0, 1], [0, layout?.displacement ?? 0])
    ...
    if (!layout) {
        return (
            <TempMeasurementDiv containerRef={ref} childArray={childArray} className={className} />
        )
    }
    ...
    return (
        <motion.div
            ...
            style={{ y, top: layout.offsetTop, height: layout.totalHeight }}
        >
            {renderItems(childArray, layout)}
        </motion.div>
    )
}
...
function AutoScrollParallax({
    ...
    const parallaxY = useTransform(scrollYProgress, [0, 1], [0, layout?.displacement ?? 0])
    ...
    return (
        <motion.div
            ref={ref}
            className={`relative flex-1 ${className ?? ""}`}
            style={{ y, height: layout.totalHeight, top: layout.offsetTop }}
        >
            {renderItems(childArray, layout)}
        </motion.div>
    )
...
export function InfiniteParallax({
    ...
    return (
        <Parallax containerRef={containerRef} speed={speed} className={className}>
            {children}
        </Parallax>
    )
}
