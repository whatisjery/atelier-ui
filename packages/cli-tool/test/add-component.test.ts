...
describe("add command", () => {
    ...
    it("installs a component with shared assets", async () => {
        await execAsync(
            `npx tsx src/index.ts add liquid-image --path ${COMPONENTS_PATH} --shared-path ${SHARED_PATH} --no-install`,
            {
                cwd: path.resolve(__dirname, ".."),
                env: { ...process.env, ATELIER_REGISTRY: `http://localhost:${port}` },
            },
        )
        expect(fs.existsSync(`${COMPONENTS_PATH}/liquid-image/liquid-image.tsx`)).toBe(true)
